...
public class SimulateTree {
    ...
    public void getTransmissionTree(EpiModel model) {

        infectionHistory iMatrix = new infectionHistory(model.getIMatrix());
        ...
        while(condition) {
            ...
            coalescentEvent coalescentEvent = findMostRecentCoalescence(currIndividuals, parentLineages, iMatrix, icoMatrix);
            ...
            parentLineages = getParentLineages(n_seqs,currIndividuals,iMatrix.parent);
...
//        Map<Integer, Double> patchBirths = new HashMap<Integer, Double>();
//        Map<Integer, Double> patchDeaths = new HashMap<Integer, Double>();
...
//                    patchBirths.put(i, iMatrix.getBirth(i));
//                    patchDeaths.put(i, iMatrix.getDeath(i));
                    patchIndices.add(i);
                }
            }
...
//                break;
    ...
    public void getTransmissionTrees(int n_segments, EpiModel model, int simNo) {
        ...
        for(int i=0; i<n_segments; i++) {
            ...
            getTransmissionTree(model);

            //System.out.println(tree.sampledLineages);
//                System.out.println("reassortant Map "+tree.reassortmentEventMap);

            //RIGHT A NOTE WHAT THIS PART IS DOING!!
            // This part refers to when the basal part of the phylogeny was fixed with random coalescence  (second part of the algorithm) - but have decided to get rid of it altogether

            //appendList(b, basal_b, n_lineages);
            // appendList(d, basal_d);
            // appendList(names, basal_names, n_lineages);
    ...
    public static void runOnePatchSim(String [] args) {
        ...
        if(args.length > 3) {
            ...
            n_lineages = Integer.parseInt(args[5].trim());
